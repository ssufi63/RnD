...
export default function TaskTable() {
  ...
  // 🔹 Load tasks (safe, works with old + new data)
  useEffect(() => {
  ...
  const buildUpdatePayload = (currentForm) => {
    ...
    return {
      task_title: currentForm.task_title ?? "",
      ...
      remarks: currentForm.remarks ?? null,
  ...
  if (!tasks || tasks.length === 0) return <p style={{ textAlign: "center" }}>No tasks found</p>;
  ...
  return (
    <div style={styles.gridWrapper}>
      {tasks.map((task) => {
        ...
        // Fallback display for Assigned By
